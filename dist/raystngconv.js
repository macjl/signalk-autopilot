...
module.exports = function (app) {
    ...
    pilot.putTargetHeadingPromise = (value) => toActionPromise((cb) => pilot.putTargetHeading(undefined, undefined, value, cb));
    pilot.putTargetHeading = (context, path, value, _cb) => {
        ...
        if (state !== 'auto') {
            ...
            var msg = util.format(heading_command, new Date().toISOString(), default_src, deviceid, padd((new_value & 0xff).toString(16), 2), padd(((new_value >> 8) & 0xff).toString(16), 2));
            sendN2k([msg]);
            return SUCCESS_RES;
        }
    };
    pilot.putStatePromise = (value) => toActionPromise((cb) => pilot.putState(undefined, undefined, value, cb));
    ...
    pilot.putTargetWindPromise = (value) => toActionPromise((cb) => pilot.putTargetWind(undefined, undefined, value, cb));
    pilot.putTargetWind = (context, path, value, _cb) => {
        ...
        if (state !== 'wind') {
            ...
            var msg = util.format(wind_direction_command, new Date().toISOString(), default_src, autopilot_dst, padd((new_value & 0xff).toString(16), 2), padd(((new_value >> 8) & 0xff).toString(16), 2));
            sendN2k([msg]);
            return SUCCESS_RES;
        }
    };
    pilot.putAdjustHeadingPromise = (value) => toActionPromise((cb) => pilot.putAdjustHeading(undefined, undefined, value, cb));
    ...
    pilot.putTackPromise = (value) => toActionPromise((cb) => pilot.putTack(undefined, undefined, value, cb));
    ...
    pilot.putAdvanceWaypointPromise = () => toActionPromise((cb) => pilot.putAdvanceWaypoint(undefined, undefined, undefined, cb));
...
function tackTo(app, deviceid, command_json) {
    ...
    if (tackTo === 'port') {
        return [
            util.format(key_command, new Date().toISOString(), default_src, everyone_dst, keys_code['-1-10'])
        ];
    }
...
function changeHeadingByKey(app, deviceid, command_json) {
    ...
    return [
        util.format(key_command, new Date().toISOString(), default_src, everyone_dst, keys_code[key])
    ];
}
function advanceWaypoint(app, deviceid) {
    return [
        util.format(raymarine_ttw_Mode, new Date().toISOString(), default_src, deviceid),
...
//# sourceMappingURL=raystngconv.js.map